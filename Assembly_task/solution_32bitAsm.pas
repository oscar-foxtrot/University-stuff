...
@nxt1: MOV EBX, dword ptr [yr]
   ...
   JMP @lbl3
@nxt2: XOR AH, AH
@lbl3: MOV DL, byte ptr [EBX+1] {make a number from 2 characters}
   SUB DL, '0'
   SHL DL, 1
   ...
   MOV word ptr [EBX], AX {res = _ _ AX}
...
@nxt3: AND AH, 01111111b
   ...
   MOV EBX, dword ptr [measure]
   SHR AL, 1
   JC @oopt2
   MOV word ptr [EBX], 'L'
   JMP @nxt4
@oopt2: MOV word ptr [EBX], 'G'
@nxt4: MOV EBX, dword ptr [volume]
   MOV byte ptr [EBX], AL
...
asm
   ...
   XOR CX, CX
   MOV EDX, dword ptr [karr]
   MOV CX, 4
@lb1: MOV EBX, dword ptr [karr]
   ADD BX, CX
@lp1: CMP BX, DX
   JE @nxt1
   MOV AL, byte ptr [EBX]
...
begin
    ...
    {if n>1 then sort(keys, sizeof(keys));
