var Vn,Ve,Pwg,Pl,V,Pdg,Bch,Pb:real;
...
assign (EmptyBST, 'D:\ФостенкоОА_Королёв\Текстовые файлы с данными (для программ)\Пустой1Bst.txt');  
assign (EmptyPB, 'D:\ФостенкоОА_Королёв\Текстовые файлы с данными (для программ)\Пустой2Pb.txt');    {В УКАЗАННОЙ ПАПКЕ БУДУТ СОЗДАНЫ ТЕКСТОВЫЕ ФАЙЛЫ}
assign (EmptyBCH, 'D:\ФостенкоОА_Королёв\Текстовые файлы с данными (для программ)\Пустой3Bch.txt');
assign (EmptyPDG, 'D:\ФостенкоОА_Королёв\Текстовые файлы с данными (для программ)\Пустой4Pdg.txt');
rewrite (EmptyBST);
rewrite (EmptyPb);
rewrite (EmptyBCH);
rewrite (EmptyPDG);
...
writeln (EmptyBST, Bst); writeln (EmptyPB, Pb); writeln (EmptyBCH, Bch); writeln (EmptyPDG, Pdg);
end;
close (VnT); close (VeT); close (LoadT); close (EmptyBST); close (EmptyPB); close (EmptyBCH); close (EmptyPDG);
end. 